...
class AirFry {
    ...
    storeCache() {
        ...
        fs_1.default.writeFile(this.cacheDir + "/cache.json", data, (err) => {
            if (err)
                throw err;
        });
    }
    ...
    writeEntryScript(script, url) {
        ...
        if (!fs_1.default.existsSync(writePath)) {
            (0, shared_1.mkdirSyncSafe)(writePath, this.outPath, { recursive: true });
        }
        let name = "index.js";
        if (url == "/")
            name = "main.js";
        ...
        (0, shared_1.writeFileSafe)(p, script, (err) => {
            if (err) {
    ...
    processGeneratorResponse(response, name, cacheName) {
        ...
        if (response.siteFiles) {
            ...
            for (const file in siteFiles) {
                ...
                (0, shared_1.writeFileSafe)(p, this.outPath, writeData, (err) => {
                    if (err) {
    ...
    renderTemplate(template, path, data) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                if (!fs_1.default.existsSync(writePath)) {
                    (0, shared_1.mkdirSyncSafe)(writePath, me.outPath, { recursive: true });
                }
                const p = path_1.default.resolve(writePath + "/index.html");
                me.state.state["html"][path] = p;
                (0, shared_1.writeFileSafe)(p, html, (err) => {
                    if (err) {
                        reject(template);
                    }
                    else {
    ...
    processScript(source, name) {
        ...
        if (source.startsWith(SCRIPT_ENTRY)) {
            ...
            if (!fs_1.default.existsSync(this.outputDir + libDir + "/" + dir)) {
                (0, shared_1.mkdirSyncSafe)(this.outputDir + libDir + "/" + dir, this.outPath, {
                    recursive: true,
                });
            }
            const p = path_1.default.resolve(this.outputDir + libDir + "/" + name + ".js");
            this.state.state["lib"][name] = p;
            (0, shared_1.writeFileSafe)(p, stripped, (err) => {
                if (err) {
