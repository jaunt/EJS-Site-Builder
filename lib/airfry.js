...
class AirFry {
    ...
    getDataFileNames(source, globList) {
        ...
        let files = (0, shared_1.getAllFiles)(resData);
        if (globList) {
            let fixedGlobs;
    ...
    storeCache() {
        ...
        if (data) {
            if (!fs_1.default.existsSync(this.cacheDir)) {
                console.log(chalk_1.default.green("Making cache dir: " + p));
                fs_1.default.mkdirSync(this.cacheDir, { recursive: true });
            }
            console.log(chalk_1.default.green("Writing cache: " + p + "/cache.json"));
    ...
    renderTemplate(template, path, data) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                me.state.errorCount++;
                console.log(chalk_1.default.red.bold(`Error rendering page: ${template}, template: ${current}, path: ${path}`));
        ...
        return new Promise(function (resolve, _) {
            ...
            toGenerate.forEach((generateData) => {
                if (me.state.generateScripts[generateData.name]) {
                    let pinger = new shared_1.Pinger(generateData.name, (id) => {
                        console.log(chalk_1.default.yellowBright("Waiting for generator to call resolve: " + id));
                    }, 3000);
                    // found a generate script -> run it
                    const generateSuccess = (response) => {
                        pinger.done();
                        console.log(chalk_1.default.yellowBright("Generator Resolved: " + generateData.name));
    ...
    compileTemplate(source, name) {
        ...
        try {
            ...
            this.state.errorCount++;
            console.log(chalk_1.default.red(`${error.message?.split("\n")[0]} in ${name}`));
        }
    }
    /// -----------------------------------------------------------------------------
    /// cueGeneration
    ...
    processPreGenerate() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (fs_1.default.existsSync(g)) {
                let pinger = new shared_1.Pinger("preGenerate", (id) => {
                    console.log(chalk_1.default.yellowBright("Waiting for generator to call resolve: " + id));
                }, 3000);
                const generateSuccess = (response) => {
                    pinger.done();
                ...
                console.log(chalk_1.default.blue(exports.PRE_GENERATE_JS + " not found, skipping."));
    ...
    processPostGenerate() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (fs_1.default.existsSync(g)) {
                let pinger = new shared_1.Pinger("postGenerate", (id) => {
                    console.log(chalk_1.default.yellowBright("Waiting for generator to call resolve: " + id));
                }, 3000);
                const generateSuccess = (response) => {
                    pinger.done();
                ...
                console.log(chalk_1.default.blue(exports.POST_GENERATE_JS + " not found, skipping."));
                resolve(); // no global data
            }
        });
    }
    ...
    updateDeps(dependencies, dependency = "") {
        ...
        if (toGenerate.length) {
            this.generatePages()
                ...
                .then(() => {
                console.log(chalk_1.default.green("Running post generate script."));
            })
                .catch((error) => {
                this.state.errorCount++;
            ...
            console.log(chalk_1.default.green("No changes to generated pages."));
        }
    }
    /// -----------------------------------------------------------------------------
    /// updateDataDeps
