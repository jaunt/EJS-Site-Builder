...
const BAD_OPTIONS = 3;
console.log(
  chalk_1.default.black.bgWhite.bold(
    "\n Air",
    chalk_1.default.white.bgBlue(" Fry \n")
  )
);
console.log(chalk_1.default.blueBright("Version " + version + "\n"));
const loggers = (0, shared_1.makeLoggers)("@ ");
...
const isOneOrTheOtherRelative = (a, b) => {
  const result =
    (0, shared_1.isRelative)(a, b) || (0, shared_1.isRelative)(b, a);
  if (result) {
    logError(
      "Directories must not contian each other: " +
        chalk_1.default.white(a + ", " + b)
    );
  }
  return result;
};
    ...
    .then(() => {
      ...
      const queueChange = (p) => {
        ...
        if (check.kind == airfry_1.PRE_GENERATE_NAME) {
          ...
          const dataFileName = path_1.default.resolve(
            dataDir + "/" + check.name
          );
      ...
      pinger = new shared_1.Pinger(
        "watcher",
        (id) => {
          // process pending dependency updates
          pinger.stop();
          airfry
            .updateDeps({ ...deps })
            .then(() => {})
          ...
          deps = {}; // clear out for the next batch
        },
        50
      );
    })
