...
nconf_1.default.argv().env().file({ file: "./templer.json" });
...
const templer = new api_1.Templer(inputDir, dataDir, outputDir, cacheDir, verbose);
    ...
    const queueChange = (p, reason = api_1.TriggerReason.Modified) => {
        ...
        if (check.kind == api_1.PRE_GENERATE_NAME) {
            ...
            if (reason == api_1.TriggerReason.Added || reason == api_1.TriggerReason.Modified) {
                ...
                templer.processDeletedTemplatePromise(templer.getTemplateFileName(check.name));
            }
        }
