...
const getOption = (opt, def) => {
    ...
    if (!result) {
        chalk_1.default.yellow("No option specified for " +
            chalk_1.default.white(opt) +
            ", using default value: " +
            chalk_1.default.green(def));
        result = def;
    }
    return def;
};
...
if (!keepOutDir) {
    if (fs_1.default.existsSync(outputDir)) {
        chalk_1.default.green("Clearing output dir: " + outputDir);
        fs_1.default.rmSync(outputDir, { recursive: true });
    }
}
if (!fs_1.default.existsSync(cacheDir)) {
    chalk_1.default.green("Making cache dir: " + cacheDir);
    fs_1.default.mkdirSync(cacheDir, { recursive: true });
}
if (watchOnly && noWatch) {
    chalk_1.default.red("Can't both watch and not watch!  Exiting.");
    (0, process_1.exit)(BAD_OPTIONS);
}
const isOneOrTheOtherRelative = (a, b) => {
    const result = (0, shared_1.isRelative)(a, b) || (0, shared_1.isRelative)(b, a);
    if (result) {
        chalk_1.default.red("Directories must not contian each other: " + chalk_1.default.white(a + ", " + b));
    }
    return result;
};
