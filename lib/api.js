...
class AirFry {
    ...
    renderTemplate(template, path, data) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                const renderInclude = function (passedData, dependency, includeData // passed with ejs include
                ) {
                    current = dependency;
                    if (dependency == "_body") {
                        dependency = template;
                    ...
                    return me.state.templates[dependency]({
                        ...
                        ...(includeData || {}),
                    }, undefined, renderInclude.bind(null, passedData));
                };
                let html;
                const inputVars = {
                    pagePath: path,
                ...
                if (me.state.frontMatter[template].wrapper) {
                    ...
                    html = me.state.templates[current](renderData, undefined, renderInclude.bind(null, renderData));
                }
                else {
                    html = me.state.templates[template](renderData, undefined, renderInclude.bind(null, renderData));
                }
        ...
        return new Promise(function (resolve, _) {
            ...
            toGenerate.forEach((generateData) => {
                if (me.getGenerateScript(generateData.name)) {
                    ...
                    const code = "((require, resolve, reject, inputs, global, getDataFileNames, cache, log, frontMatterParse, dataDir) =>  {" +
                        me.getGenerateScript(generateData.name) +
                        "})";
                    me.expireCache();
                    try {
                        vm_1.default.runInThisContext(code)(require, generateSuccess, generateError, inputs, me.getGlobalDataAccessProxy(generateData.name), me.getDataFileNames.bind(me, generateData.name), me.state.cache[me.getCacheName(generateData.name)], me.scriptLogger.bind(null, generateData.name), front_matter_1.default, path_1.default.resolve(me.dataDir));
                    }
                    catch (error) {
                        me.state.errorCount++;
    ...
    getCacheName(name) {
        ...
        if (ref) {
            if (this.state.generateScripts[ref]) {
                if (this.verbose) {
                    log(chalk_1.default.yellow("using referencecache '" + ref + "' for '" + name + "'"));
                }
                return "_" + ref;
            }
        }
