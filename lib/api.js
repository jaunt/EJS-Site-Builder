        ...
        const _generateTemplate = (generateData) => {
            return new Promise(function (resolve, reject) {
                ...
                if (me.getGenerateScript(generateData.name)) {
                    ...
                    const code = `((require, resolve, reject, generatePage, inputs, getDataFileNames, cache, log, frontMatterParse, dataDir, renderTemplate) =>  { ` +
              ...
              try {
                ...
                reject(error);
                    ...
                    me.expireCache();
                    try {
                        vm_1.default.runInThisContext(code, {
                            filename: me.getGenerateScriptPath(generateData.name),
                        })(require, generateDone, // set done
                        generateError, generatePagesRequest, // render array of pages and continue
                        inputs, me.getDataFileNames.bind(me, generateData.name), me.state.cacheData, me.scriptLogger.bind(null, generateData.name), front_matter_1.default, path_1.default.resolve(me.dataDir), renderTemplateRequest);
                    }
                    catch (error) {
                        me.state.errorCount++;
